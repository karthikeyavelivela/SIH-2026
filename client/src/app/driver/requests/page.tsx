...
export default function DriverRequestsPage() {
  ...
  // useIncomingOffer's respond() resolves (never rejects) with {ok,error}
  // over the socket ack — that result was previously discarded entirely,
  // so a failed accept (lost the race, rating gate) looked identical to a
  // successful one from the UI's point of view.
  ...
  return (
    <div className="min-h-screen bg-fy-bone pb-24">
      ...
      <p className="text-sm text-fy-ink-soft mb-4">{t('subtitleTruck')}</p>

      <Link
        href="/driver/loadboard"
        className="flex items-center gap-2.5 mb-6 p-3 rounded-card bg-fy-brown/10 text-primary-700 hover:bg-fy-brown/15 transition-colors duration-fast text-sm font-semibold"
      >
