...
// hamburger drawer) per DESIGN_INVENTORY's classification of
// partner_fleet_dashboard as a SideNav-driven console screen, not a
// BottomTabNav-driven mobile worker screen.
export default function FleetOwnerLayout({ children }: { children: React.ReactNode }) {
  const t = useTranslations('consoleLayout');
  const NAV_ITEMS = [
    { href: '/fleet-owner/dashboard', label: t('dashboard'), icon: <HomeIcon className="w-5 h-5" /> },
  ...
  return (
    <div className="min-h-screen flex bg-fy-bone text-fy-ink">
      <aside className="w-64 shrink-0 border-r border-fy-muted/10 bg-fy-card p-6 hidden md:flex md:flex-col">
        <div className="mb-10 flex items-start justify-between gap-2">
          <div className="flex items-center gap-2.5">
            <span className="w-9 h-9 rounded-full bg-fy-brown-soft/20 text-fy-brown flex items-center justify-center flex-shrink-0">
      ...
      <div className="md:hidden fixed top-0 inset-x-0 z-40 flex items-center justify-between px-4 py-3 bg-fy-card border-b border-fy-muted/10 shadow-sm">
        <p className="font-heading font-bold text-fy-brown">{t('fyroFleet')}</p>
