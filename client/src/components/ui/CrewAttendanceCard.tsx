...
export function CrewAttendanceCard({ name, photoUrl, status, currentTask, onClick }: CrewAttendanceCardProps) {
  ...
  return (
    <Wrapper
      ...
      onClick={onClick}
      className={`w-full flex items-center gap-4 py-2 text-left ${onClick ? 'active:bg-fy-well rounded-control -mx-2 px-2 transition-colors' : ''}`}
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {currentTask && <p className="text-xs text-fy-ink-soft truncate">{currentTask}</p>}
      </div>
