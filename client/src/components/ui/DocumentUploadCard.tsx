...
export function DocumentUploadCard({ label, status, fileName, rejectionReason, onUpload }: DocumentUploadCardProps) {
  ...
  return (
    <div className="fy-surface-card flex items-center gap-4">
      <div className="w-11 h-11 rounded-full bg-fy-edge flex items-center justify-center text-fy-ink-soft flex-shrink-0">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-sm font-semibold text-fy-ink truncate">{label}</p>
        {fileName && <p className="text-xs text-fy-ink-soft truncate">{fileName}</p>}
        {status === 'rejected' && rejectionReason && <p className="text-xs text-fy-error mt-0.5">{rejectionReason}</p>}
      </div>
      <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
        ...
        {canUpload && (
          <>
            ...
            />
            <button type="button" onClick={() => inputRef.current?.click()} className="text-xs font-semibold text-fy-brown hover:underline">
