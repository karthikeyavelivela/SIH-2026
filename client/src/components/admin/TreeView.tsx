...
export function TreeView({ adminName, managers, onEditManager }: TreeViewProps) {
  ...
  return (
    <div className="flex flex-col items-start gap-4 w-full min-w-0" role="tree" aria-label="Admin organization tree">
      <Card
        elevation="raised"
        className="border-2 border-fy-brown/30 bg-gradient-to-br from-fy-brown/5 to-transparent"
        role="treeitem"
        aria-level={1}
      >
        <p className="text-xs font-bold uppercase tracking-[0.15em] text-fy-brown mb-1.5">{t('adminRoot')}</p>
        <p className="font-heading font-semibold text-lg">{adminName}</p>
      </Card>
      <div
        className="pl-8 border-l-2 border-fy-hairline ml-4 flex flex-col gap-3 w-full"
        role="group"
        aria-label="Managers"
      >
        {managers.length === 0 && (
          <p className="text-sm text-fy-muted py-4">{t('noManagersYet')}</p>
        )}
        {managers.map((m) => (
          <Card
            key={m._id}
          ...
          >
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="text-xs text-fy-muted whitespace-nowrap">{m.phone}</span>
            </div>
            ...
            {onEditManager && (
              <div className="pt-2 border-t border-fy-hairline">
