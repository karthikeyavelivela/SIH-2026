...
4. **Geocoding proxy**: the client needs address autocomplete, but Nominatim's usage policy (User-Agent, rate limits) is already handled server-side (Phase 1's `geocode.service.ts`, unwired until now). Expose it via a new `GET /api/geocode?q=` route rather than calling Nominatim directly from the browser — rate-limited AND auth-gated (`verifyJwt`, any role), not public. Corrected during implementation review: the only real consumer (address entry during booking) is always behind login already, so there's no genuine use case needing anonymous access, and leaving it open would make the endpoint an unlimited free Nominatim relay for anyone on the internet (a per-IP rate limit alone is trivially defeated by rotating source IPs) — risking Nominatim banning the app's egress IP and breaking geocoding for every real user at once.
