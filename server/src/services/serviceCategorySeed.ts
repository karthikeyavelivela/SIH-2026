...
export const CATEGORIES: SeedCategory[] = [
  ...
  {
    // Farm labour: sowing, transplanting, weeding, harvest, threshing. Priced
    // per worker through the same fare rules as general labour, so it is
    // held to the same statutory wage floor; served only by workers who
    // signed up for farm work (workerEligibility.ts).
    name: 'Farm Labour',
    slug: 'agri_labour',
    icon: 'LeafIcon',
    accentColor: 'secondary',
    pricingUnit: 'per_worker',
    requiredSkills: ['agricultural'],
    requiresVehicle: false,
    requiresMaterials: false,
...
/**
 * Creates any category in CATEGORIES that does not exist yet, by slug, and
 * returns how many it created. Never modifies an existing row — an admin's
 * edits to a category survive every restart. Run at boot, because the
 * category list used to change only when somebody remembered to run the
 * seed script against production.
 */
export async function ensureServiceCategories(): Promise<number> {
  let created = 0;
  for (const c of CATEGORIES) {
    if (await ServiceCategory.exists({ slug: c.slug })) continue;
