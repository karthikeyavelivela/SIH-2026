...
/**
 * The training curriculum, and the code that makes sure it is actually in
 * the database.
 *
 * This lived only in a one-off script, which is why production served three
 * cargo modules and none of the four trade ones: the script had never been
 * run against it, and nothing in the app noticed. A curriculum that exists
 * in the repository but not in the database is not a feature.
 *
 * So the seeding runs at boot now, idempotently, upserting by `order` and
 * logging only what it actually had to create. The script still exists and
 * still works — it calls straight into this.
 */
...
/**
 * Creates any missing module. Never updates or deletes an existing one:
 * an admin who edited a module's content in production should not have it
 * silently overwritten by a deploy.
 */
export async function ensureTrainingModules(): Promise<number> {
  let created = 0;
  for (const m of MODULES) {
    const existing = await TrainingModule.findOne({ order: m.order }).select('_id');
...
export { MODULES as TRAINING_MODULES };
